...
export default function ImageDropZone({ shape, dimensions, onFileUpload, disabled }: ImageDropZoneProps) {
   ...
   */
  const uploadFile = async (file: File): Promise<{ fileKey: string; publicUrl: string } | null> => {
    try {
      ...
      const uploadUrlResponse = await fetch(`${backendUrl}/store/stickers/upload-url`, {
        method: 'POST',
        headers,
        body: JSON.stringify({
          filename: file.name,
      ...
      if (!uploadUrlResponse.ok) {
        ...
        try {
          ...
          throw new Error('Failed to get upload URL. The server returned a non-JSON response.');
        }
      }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-center">
        ...
        >
          ...
          <div
            className={clsx(
              'absolute inset-0 transition-colors overflow-hidden',
              {
                'border-2 border-dashed border-neutral-700 hover:border-neutral-500': !isDragActive && !disabled && !isUploading,
                ...
                */}
                {shape === 'diecut' && hasTransparency && outlineDataUrl ? (
                  ...
                  <Image
                    ...
                    style={{
                      objectFit: 'contain',
                      transform: shape === 'diecut' ? undefined : `scale(${scale})`,
                      objectPosition: '50% 50%',
                      transition: shape === 'diecut' ? undefined : 'transform 0.2s ease-out',
                      ...(shape === 'diecut' && !hasTransparency ? { clipPath: starClipPath } : {}),
                    }}
                    className={clsx(
